# Read me


### 요구사항 명세서

<img width="1217" height="192" alt="로또 요구사항 명세서" src="https://github.com/user-attachments/assets/4b45d3f8-306e-4388-9fc4-5cdd2d9dc713" />

요구사항 분석
로또 홈페이지는 사용자가 로또를 구매하고, 당첨 번호를 확인하며, 이력 및 통계 정보를 조회할 수 있는 웹 서비스입니다. 주요 이해관계자는 일반 사용자, 사이트 관리자입니다.​

핵심 목표
온라인에서 로또를 간편하게 구매/관리

최신 및 과거 당첨 번호 제공

당첨 번호 대조 및 등수 결과 제공

사용자 맞춤 정보 및 서비스 제공



### 로또 UI

<img width="1222" height="217" alt="로또 UI" src="https://github.com/user-attachments/assets/702b1ca5-6ae5-4cd8-b90b-031d516d1799" />

<img width="2551" height="1385" alt="image" src="https://github.com/user-attachments/assets/36b5542a-3a95-4d3f-af84-df024eddbdf0" />


UI/UX 설계
주요 화면 및 구성
회원가입/로그인:
간단한 입력폼, 소셜 로그인 옵션(선택), 직관적인 진행 상태 표시

로또 구매:
로또 번호는 랜덤 자동 배정, 1~5세트 선택 입력, 구매 내역 실시간 표시

로또 당첨 확인:
회차별 검색 기능, 명확한 당첨 번호 표시, 등수별 결과(애니메이션 또는 컬러 하이라이트)

구매/당첨 내역:
표/카드 기반 리스트, 내역 클릭시 상세정보 팝업 또는 이동

관리자 전용 화면:
당첨 번호 등록/수정, 회원/구매관리, 직관적인 대시보드




### API 리스트


<img width="1223" height="327" alt="API 리스트" src="https://github.com/user-attachments/assets/48ebd7ea-0ebb-4c54-ab38-0b1dc49ba2c1" />

기능 요구 사항

기본적으로 제약조건은 기존 프리코스 3주차에 실행 했던 기능 요구 사항을 따름.
API기반으로 Annotation을 사용해서 get set 메서드 구현.
MongoDB와 연결해서 데이터를 주고 받을 수 있는지
Reactjs로 ui를 구성해서 Springboot로 기능 세팅.



project.

2025 11 15 - 프로젝트 생성 및 리액트 프로젝트 빌드
2025 11 16 - 기존 프로젝틈 프레임 작성.
2025 11 17 mongodb 채택 및 테이블 로직 작성
2025 11 18 백앤드 리액트 연결 / 스프링부트 로그아웃 로그인 기능 몽고 db 연동 및 테스트 . 도커 환경 컨테이너 작성
2025 11 22 페이지 Redirect 시 로그아웃 되는 현상. jwt 토큰으로 받아서 로그아웃이 안되게 수정
2025 11 23 마이페이지 및 전체적인 레이아웃 및 css 수정 . 추가로 lottodraw 테이블 생성 및 백앤드와 연결하여 당첨내역 조회 기능 구현

페이지 레이아웃

메인 페이지

<img width="2542" height="1355" alt="image" src="https://github.com/user-attachments/assets/66ad65c2-a51d-49f8-b394-88d216f69f8a" />

로그인 페이지
<img width="2559" height="1334" alt="image" src="https://github.com/user-attachments/assets/136cc396-719d-4a40-8346-52b7345e8afe" />

회원 가입 페이지

<img width="2548" height="1337" alt="image" src="https://github.com/user-attachments/assets/469f5707-73ef-4ce4-9fe4-42a81019486c" />

구매 페이지 구매는 회차별로 구매

1000원 단위 입력
<img width="2550" height="1323" alt="image" src="https://github.com/user-attachments/assets/292495c2-4d7b-430f-affe-edf6238f09c8" />

당첨 회차 당첨 번호 확인 페이지

<img width="2549" height="1326" alt="image" src="https://github.com/user-attachments/assets/797a7389-6dda-4596-9ff4-f21bd084b4cc" />

구매내역 페이지

당첨 번호와 내가 구매를 대조하여 당첨금액 및 
당첨 번호 - 몇등인지 표시
<img width="2555" height="1332" alt="image" src="https://github.com/user-attachments/assets/15361303-82ba-42e7-91d2-bace1c902283" />

마이페이지

비밀변호 변경 가능.
<img width="2552" height="1325" alt="image" src="https://github.com/user-attachments/assets/f66c38d3-3ce4-4093-b33b-5a8aae78347e" />


### 프로젝트 마무리 후 느낀점.

프로젝트를 진행하면서 느낀점과 반성할 점.

이번 프로젝트에서는 우테코에서 제시한 3가지 중 첫번째인 낯선 프로그램 언어를 사용하는 걸 햇는데 java 기반인 springboot 와 react.js를 사용하였습니다.

사용을 하지 않던 언어라서 어노테이션이나 getter setter 메서드들을 찾아보면서 프로젝트를 구성해 보았고 

로그인과 로그아웃 로직 구현 및 그거에 따른 데이터 플로우를 경험할 수 있었습니다. 추가로 db 와의 연결을 통해서 데이터를 통신하고 저장해서 crud 의 작업을 하는 법을 알았고

mongodb에서는 json 파일을 통해서 http 통신을 통하여 통신이 된다는 것을 알았습니다.

추가로 postman 이라는 프로그램을 사용해봤는데 실시간으로 내 프로젝트의 통신과 어떤 통신 오류가 나는지도 깨달을 수 있었습니다.

이런 여러 좋은 경험을 하였지만 반성해야할 점은 좀 더 Ai의존성을 줄이고 다시 코드를 복기하면서 다음에는 ai를 배제하고 만들더라도 수월하게 만들 수 있도록 해야겠다는 점을 느꼈습니다.

### 보안해야할 점

## 토큰 값으로 30분 후 로그인 페이지가 로그아웃이 되는 로직 구현

## 레이아웃 및 css 다듬기

## 실제로 결제가 되는 로직이 어떻게 되는지 알아보고 추가로 구현가능 하면 구현.






